...
model = pickle.load(open('/content/drive/MyDrive/Colab Notebooks/Customer Feedback Classification/absa/models/LSTM/Hyperparam Tuning/model0.01', 'rb'))
...
for sen in range(len(test)):
  ...
  label.append({'Negative': np.amax(predict[:, 0]), 'Neutral': np.amax(predict[:, 1]), 'Positive': np.amax(predict[:, 2])})
